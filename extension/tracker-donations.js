...
async function updateToReadDonations() {
    ...
    try {
        ...
        nodecg_1.get().log.warn('[Tracker] Error updating to read donations');
        nodecg_1.get().log.debug('[Tracker] Error updating to read donations:', err);
...
async function markDonationAsRead(donationID) {
    try {
        const resp = await needle_1.default('get', `https://${config.address}/edit/?type=donation&id=${donationID}`
            + '&readstate=READ&commentstate=APPROVED', {
            cookies: tracker_1.getCookies(),
        });
        if (resp.statusCode === 200) {
            nodecg_1.get().log.info(`[Tracker] Successfully marked donation ${donationID} as read`);
        }
        else {
        ...
        nodecg_1.get().log.warn(`[Tracker] Error marking donation ${donationID} as read`);
        nodecg_1.get().log.debug(`[Tracker] Error marking donation ${donationID} as read:`, err);
    }
