...
rabbitmq_1.mq.evt.on('bigbuttonPressed', async (data) => {
    ...
    if (data.time.unix > (Date.now() / 1000) + 10) {
        nodecg_1.get().log.warn('[Timer] Big button unix timestamp in the future, this is bad!');
        return;
    }
...
sc.on('timerStarted', () => logging_1.logTimerChange('started'));
sc.on('timerPaused', () => logging_1.logTimerChange('paused'));
sc.on('timerResumed', () => logging_1.logTimerChange('resumed'));
sc.on('timerStopped', () => logging_1.logTimerChange('finished'));
sc.on('timerReset', () => logging_1.logTimerChange('reset'));
sc.on('timerEdited', () => logging_1.logTimerChange('edited'));
sc.on('timerTeamStopped', (id) => logging_1.logTimerChange('team_finished', id));
sc.on('timerTeamUndone', (id) => logging_1.logTimerChange('team_undid_finish', id));
