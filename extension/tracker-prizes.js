...
async function updatePrizes() {
    try {
        const resp = await needle_1.default('get', `https://${config.address}/search/?event=${tracker_1.eventInfo[0].id}&type=prize`, {
        ...
        nodecg_1.get().log.warn('[Tracker] Error updating prizes');
        nodecg_1.get().log.debug('[Tracker] Error updating prizes:', err);
