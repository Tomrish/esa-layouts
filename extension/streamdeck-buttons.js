    ...
    streamdeck_1.default.on('keyUp', async (data) => {
        ...
        if (data.action === 'com.esamarathon.streamdeck.twitchads'
            ...
            try {
                ...
                nodecg_1.get().log.warn('[Stream Deck Buttons] Could not run Twitch commercials');
                nodecg_1.get().log.debug('[Stream Deck Buttons] Could not run Twitch commercials:', err);
            }
        }
        ...
        if (data.action === 'com.esamarathon.streamdeck.ttsdonations') {
            ...
            if (donation) {
                text_to_speech_1.speak(donation);
                tracker_donations_1.markDonationAsRead(donation.id);
            }
        }
